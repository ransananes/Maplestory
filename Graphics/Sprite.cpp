...
namespace ms
{
	...
	Sprite::Sprite(nl::node src, Point<int16_t> pos, bool xaxis, bool yaxis)
	{
		float xscale, yscale = 1;
		Animation sp = Animation(src);
		Point<int16_t> VIEWSIZE = Constants::Constants::get().get_viewsize();
		if(xaxis) xscale = (float)VIEWSIZE.x() / (float)sp.get_dimensions().x();
		if(yaxis) yscale = (float)VIEWSIZE.y() / (float)sp.get_dimensions().y();
		animation = src;
		stateargs = DrawArgument(pos, xscale, yscale);
	}
	Sprite::Sprite() {}
