...
namespace ms
{
	UIWorldSelect::UIWorldSelect() : UIElement(Point<int16_t>(0, 0), Point<int16_t>(VIEWSIZE.x()/2, VIEWSIZE.y()/2)), worldcount(0), world_selected(false)
	{
		...
		sprites.emplace_back(WorldSelectBG["24"], Point<int16_t>(VIEWSIZE.x() / 2, VIEWSIZE.y() / 2), true, true);
