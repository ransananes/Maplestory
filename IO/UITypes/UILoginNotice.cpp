...
namespace ms
{
	UILoginNotice::UILoginNotice(uint16_t message, std::function<void()> okhandler, std::function<void()> cancelhandler) : okhandler(okhandler), cancelhandler(cancelhandler)
	{
		...
		switch (message)
		{
			...
			case Message::NOT_REGISTERED:
			case Message::WRONG_PASSWORD:
			case Message::WRONG_EMAIL:
			case Message::WRONG_GATEWAY:
		...
		sprites.emplace_back(backgrnd, Point<int16_t>(0, 0));
		Sprite _backgrnd = Sprite(backgrnd);
		sprites.emplace_back(Notice["text"][message], Point<int16_t>(_backgrnd.width() / 3.2, 25));
		...
		if (message == Message::DELETE_CONFIRMATION || message == Message::CASH_ITEMS_CONFIRM_DELETION)
		{
			...
			buttons[Buttons::YES] = std::make_unique<MapleButton>(Notice["BtYes"], Point<int16_t>(130, 130));
			buttons[Buttons::NO] = std::make_unique<MapleButton>(Notice["BtNo"], Point<int16_t>(130, 106));
		}
		else
		{
			buttons[Buttons::YES] = std::make_unique<MapleButton>(Notice["BtYes"], Point<int16_t>(_backgrnd.width() / 2.3, _backgrnd.height() / 1.5));
		}
	...
	UIQuitConfirm::UIQuitConfirm()
	{
		...
		sprites.emplace_back(notice["text"][UILoginNotice::Message::CONFIRM_EXIT], Point<int16_t>(110, 50));

		buttons[Buttons::BT_OK] = std::make_unique<MapleButton>(notice["BtYes"], Point<int16_t>(100, 150));
		buttons[Buttons::BT_CANCEL] = std::make_unique<MapleButton>(notice["BtNo"], Point<int16_t>(200, 150));

		position = Point<int16_t>(VIEWSIZE.x()/3, VIEWSIZE.y()/3);
		dimension = Texture(backgrnd).get_dimensions();
	}
	...
	UIClassConfirm::UIClassConfirm(uint16_t selected_class, bool unavailable, std::function<void()> okhandler) : okhandler(okhandler)
	{
		...
		{
			buttons[Buttons::OK] = std::make_unique<MapleButton>(type["BtOK"], Point<int16_t>(100, 200));
